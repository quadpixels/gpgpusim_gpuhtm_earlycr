...
public: 
   ...
   void reset();
   void printActiveAndValidationDoneMask();
...
private:
    ...
    void process_input( mem_fetch *mf, unsigned time );
    ...
    void process_commit_op_reply(mem_fetch *mf, const cu_mem_acc &mem_op, unsigned time);
    ...
    void check_and_advance_commit_ptr(unsigned long long time); 
    void check_and_advance_commit_ptr_vwait_nostall(unsigned long long time); 
    void check_and_advance_retire_ptr(unsigned long long time); 
